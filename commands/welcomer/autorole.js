...
async function addAutoRole({ guild, client }, role) {
    ...
    if (role) {
        if (!guild.me.permissions.has('MANAGE_ROLES'))
            return `<a:crs:1304817967413854209> | I don't have the \`MANAGE_ROLES\` permission`
        if (guild.me.roles.highest.position < role.position)
            return `<a:crs:1304817967413854209> | I don't have the permissions to assign this role`
        if (role.managed)
            return `<a:crs:1304817967413854209> | This role is managed by an integration.`
    }
    if (!role) {
        settings.autorole = []
        await settings.save()
        return `<a:tk:1304818061034917979> | Autorole module was successfully disabled.`
    }
    if (settings.autorole.includes(role.id))
        return `<a:crs:1304817967413854209> | This role is already present in the autorole config.`
    if (settings.autorole.length == 10)
        return `<a:crs:1304817967413854209> | Maximum 10 roles can be set for Auto Roles.`
    else settings.autorole.push(role.id)
    await settings.save()
    return `<a:tk:1304818061034917979> | Successfully **added** <@&${role.id}> to Autorole Config.`
}
...
async function removeAutoRole({ guild, client }, role) {
    ...
    if (!settings.autorole.includes(role.id))
        return `<a:crs:1304817967413854209> | This role is not present in the autorole config.`
    if (settings.autorole.length == 0)
        return `<a:crs:1304817967413854209> | There are no Autoroles in my config.`
    else settings.autorole = settings.autorole.filter((r) => r !== role.id)
    await settings.save()
    return `<a:tk:1304818061034917979> | Successfully **removed** <@&${role.id}> from Autorole Config.`
}

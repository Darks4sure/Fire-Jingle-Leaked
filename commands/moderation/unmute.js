...
async function untimeout(issuer, target, reason) {
    ...
    if (typeof response === 'boolean')
        return getEmbed(
            `<:IconTick:1245261305561223199> | Successfully unmuted <@${target.user.id}>!`,
            client
        )
    if (response === 'BOT_PERM')
        return getEmbed(
            `<a:crs:1304817967413854209> | I don't have enough permissions to unmute <@${target.user.id}>.`,
            client
        )
    else if (response === 'MEMBER_PERM')
        return getEmbed(
            `<a:crs:1304817967413854209> | You don't have enough permissions to unmute <@${target.user.id}>.`,
            client
        )
    else if (response === 'NO_TIMEOUT')
        return getEmbed(
            `<a:crs:1304817967413854209> | <@${target.user.id}> is not muted only!`,
            client
        )
    else
        return getEmbed(
            `<a:crs:1304817967413854209> | I don't have enough perms to unmute <@${target.user.id}>.`,
            client
        )
}

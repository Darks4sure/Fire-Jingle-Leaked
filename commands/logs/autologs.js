...
module.exports = {
    ...
    run: async (client, message, args) => {
            ...
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        ...
                        .setFooter(`Note : If you wannna setup logging again use ${message.guild.prefix}logsreset & delete all existinglog channels of Fire-Advance`, client.user.displayAvatarURL())
                ]
            });
        ...
        try {
            let category = message.guild.channels.cache.find(c => c.type === 'GUILD_CATEGORY' && c.name === 'Fire-Advance-LOGS');

            if (!category) {
                category = await message.guild.channels.create('Fire-Advance-LOGS', {
                    type: 'GUILD_CATEGORY',
    ...
    return message.channel.send({
        embeds: [
            new MessageEmbed()
                ...
                .setDescription('All necessary logging channels have been successfully created under the "Fire-Advance LOGS" category.')

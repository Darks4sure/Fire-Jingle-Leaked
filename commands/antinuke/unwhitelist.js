...
`
        ...
        const uwl = new MessageEmbed()
            ...
            .addFields([
                {
                    name: `__**Usage**__`,
                    value: `<:dot:1314799225120227388> \`${message.guild.prefix}unwhitelist @user/id\`\n<:dot:1314799225120227388> \`${message.guild.prefix}uwl @user\``
                }
            ])
...
Current Status : <a:crs:1304817967413854209><a:tk:1304818061034917979>
                        ...
                        return message.channel.send({
                            embeds: [
                                new MessageEmbed()
                                    .setColor(client.color)
                                    .setDescription(
                                        `<a:crs:1304817967413854209> | <@${user.id}> is not a whitelisted member.`
                                    )
                            ]
                        })
                    } else {
                                ...
                                return message.channel.send({
                                    embeds: [
                                        new MessageEmbed()
                                            .setColor(client.color)
                                            .setDescription(
                                                `<a:tk:1304818061034917979> | Successfully removed <@${user.id}> from whitelisted user.`
                                            )
                                    ]
                                })
                            }

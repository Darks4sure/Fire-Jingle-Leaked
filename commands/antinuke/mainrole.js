...
async function addMainrole({ guild, client }, role) {
    ...
    if (!role) {
        settings.mainrole = []
        await settings.save()
        return `<a:tk:1304818061034917979> | **Mainrole module was successfully disabled.**`
    }
    if (settings.mainrole.includes(role.id))
        return `<a:crs:1304817967413854209> | **This role is already present in the mainrole config.**`
    if (settings.mainrole.length == 5)
        return `<a:crs:1304817967413854209> | **Maximum 5 roles can be set for Mainroles.**`
    else settings.mainrole.push(role.id)
    await settings.save()
    return `<a:tk:1304818061034917979> | **Successfully **added** <@&${role.id}> to Mainrole Config.**`
}
...
async function removeMainrole({ guild, client }, role) {
    ...
    if (!settings.mainrole.includes(role.id))
        return `<a:crs:1304817967413854209> | **This role is not present in the mainrole config.**`
    if (settings.mainrole.length == 0)
        return `<a:crs:1304817967413854209> | There are no Mainrole in my config.`
    else settings.mainrole = settings.mainrole.filter((r) => r !== role.id)
    await settings.save()
    return `<a:tk:1304818061034917979> | **Successfully **removed** <@&${role.id}> from Mainrole Config.**`
}
...
async function listMainrole({ guild, client }) {
    ...
    let roles = settings.mainrole
        .map((role) => `<:dot:1314799225120227388> <@&${role}> (${role})`)
        .join('\n')
    roles =

...
// Emoji constants
let enable = `<:green:1314797962848632872><:crred:1314798393159057458>`;
let disable = `<:red:1314797954913271818><:tkgreen:1314797948030156820`;
let protect = `<:anti:1304657090484240385>`;
    ...
    run: async (client, message, args) => {
            ...
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        ...
                        .setDescription(`<a:crs:1304817967413854209> | **Your Server Doesn't Meet My 5 Member Criteria**`)
                ]
            });
        }
        ...
        if (!own && !check) {
            return message.channel.send({
                embeds: [
                    new MessageEmbed()
                        .setColor(client.color)
                        .setDescription(`<a:crs:1304817967413854209> | **Only the server owner or an extra owner with a higher role than mine is authorized to execute this command.**`)
                ]
            });
        }
        ...
        if (!own && !(message?.guild.members.cache.get(client.user.id).roles.highest.position <= message?.member?.roles?.highest.position)) {
            const higherole = new MessageEmbed()
                .setColor(client.color)
                .setDescription(`<a:crs:1304817967413854209> | **Only the server owner or extra owner with a higher role than mine can execute this command.**`);
            ...
            if (isActivatedAlready) {
                ...
                let msg = await message.channel.send({
                    embeds: [
                        new MessageEmbed()
                            .setColor(client.color)
                            .setDescription(`<a:tk:1304818061034917979> | **Initializing Quick Setup!**`)
                    ]
                });
                // Simulate setup
                ...
                if (message.guild.roles.cache.size > 285)
                    return message.reply(`I Won't Able To Create \`Fire-Advance Dominance\` Cause There Are Already 285 Roles In This Server`);
                ...
                let createdRole = await message.guild.roles.create({
                    ...
                    reason: 'Kyzex-Advance Role For Ubypassable Setup',
